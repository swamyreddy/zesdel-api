...
// ── Helper: issue tokens ──────────────────────────────────────────────────────
async function issueTokens(userId: any, role: string, res: Response, message: string) {
...
export const sendOtpHandler = asyncHandler(async (req: Request, res: Response) => {
  ...
  const recentCount = await OTP.countDocuments({
    phone, purpose, expiresAt: { $gt: new Date() },
  });
  if (recentCount >= 3) {
...
export const verifyLoginOtp = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!valid) {
    ...
    throw new AppError(`Invalid OTP. ${rem} attempt${rem !== 1 ? 's' : ''} remaining.`, 400);
  }
