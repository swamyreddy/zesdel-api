...
export const sendOTP = async (phone: string, otp: string): Promise<void> => {
    ...
    console.log("Check 2: In production, sending OTP via MSG91  ");
    ...
    console.log("Check 3: In production, sending OTP via MSG91  ");
    // MSG91 expects phone without + e.g. 919876543210
    const mobileNumber = phone.replace(/^\+/, "");

    try {
        console.log("Check 4: In production, sending OTP via MSG91  ");
        ...
        console.log("Check 5: In production, sending OTP via MSG91  ");
        const data = (await res.json()) as { type: string; message: string };

        if (data.type === "success") {
            console.log("Check 6: In production, sending OTP via MSG91  ");
            console.log(`MSG91 OTP sent to ${phone}`);
        } else {
            console.log("Check 7: In production, sending OTP via MSG91  ");
